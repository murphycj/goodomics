---
title: "Server"
description: "Run the optional FastAPI, MCP, database, and React dashboard server."
---

# Server

Start the server with `uv run --package goodomics goodomics serve --reload` for local development or without reload in production. The FastAPI API is available under `/api/v1/*`, and `/` serves the built React dashboard (`packages/goodomics/src/goodomics/server/web/static/index.html`).

If dashboard assets are missing, `/` returns setup guidance by default. To proxy dashboard requests to Vite during development, set `GOODOMICS_DASHBOARD_DEV_URL` (for example `http://127.0.0.1:5173`) and run the dashboard dev server.
